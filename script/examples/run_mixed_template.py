...
from openprompt.prompts import MixedTemplate, SoftTemplate, Templ

mixed_template = MixedTemplate(model=plm, tokenizer=tokenizer,
                               text='{"placeholder":"text_a"} {"soft": "Question:"} {"placeholder":"text_b"}? {"soft": "Is it correct?"} {"mask"}')
wrapped_example = mixed_template.wrap_one_example(dataset['train'][0])
print(wrapped_example)

# mixed_template2 = MixedTemplate(model=plm, tokenizer=tokenizer,
#                                 text='{"placeholder":"text_a"} {"soft"} {"soft"} {"soft"} {"placeholder":"text_b"} {"soft"} {"mask"}.')
# wrapped_example = mixed_template2.wrap_one_example(dataset['train'][0])
# print(wrapped_example)
...
train_dataloader = PromptDataLoader(dataset=dataset["train"], template=mixed_template, tokenizer=tokenizer,
...
use_cuda = True
prompt_model = PromptForClassification(plm=plm, template=mixed_template, verbalizer=myverbalizer, freeze_plm=False)
if use_cuda:
...
# %%
validation_dataloader = PromptDataLoader(dataset=dataset["validation"], template=mixed_template, tokenizer=tokenizer,
                                         tokenizer_wrapper_class=WrapperClass, max_seq_length=256, decoder_max_length=3,
