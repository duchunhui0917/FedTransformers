...
def plot_class_samples(mtx, **kwargs):
    # plt.style.use('ggplot')
    n, c = mtx.shape[0], mtx.shape[1]
    colors = plt.get_cmap('Spectral')(np.linspace(0, 1, c))

    distributions_tran = mtx.transpose()
    s = 0
    for j in range(c):
        plt.barh(range(n), distributions_tran[j], left=s, color=colors[j])
        s += distributions_tran[j]
    plt.xlabel('number of samples')
    plt.ylabel('clients')
    plt.yticks(range(n))
    # if 'label_names' in kwargs:
    #     plt.legend(kwargs['label_names'], loc='uper ')
    plt.show()
...
def plot_hist(train_datasets, bins=None):
    nums = [len(dataset) for dataset in train_datasets]
