...
def mmd(source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    ...
    kernels = guassian_kernel(source, target,
                              ...
                              fix_sigma=fix_sigma)
    xx = np.mean(kernels[:batch_size, :batch_size])  # Source<->Source
    yy = np.mean(kernels[batch_size:, batch_size:])  # Target<->Target
    xy = np.mean(kernels[:batch_size, batch_size:])  # Source<->Target
    yx = np.mean(kernels[batch_size:, :batch_size])  # Target<->Source
