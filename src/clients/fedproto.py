...
class FedProtoClient(BaseClient):
    def __init__(self, client_id, dataset, model, args):
        super(FedProtoClient, self).__init__(client_id, dataset, model, args)
        self.mu = args.mu
    ...
    def train_model(self, ite=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, data in enumerate(t):
                ...
                with torch.no_grad():
                    _, global_features, _, _ = global_model(tmp_data)
                args = {'proto_ent': global_features[1]}

                tmp_data = copy.deepcopy(data)
                for key, val in data.items():
                    tmp_data[key] = val.cuda()
                labels, features, logits, losses = model(tmp_data, args=args)
